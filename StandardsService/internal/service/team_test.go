...
func setupTeamSvc() (*service.TeamService, *testutil.TeamRepoMock, *testutil.AuthMock) {
	auth := testutil.NewAuthMock()
	teams := testutil.NewTeamRepoMock()
	svc := service.NewTeamService(teams, auth)
	return svc, teams, auth
}
...
func seedAuth(auth *testutil.AuthMock) {
	...
	auth.SetGroupAccess(groupID, userMaintID, 40) // Maintainer
	auth.SetGroupAccess(groupID, userDevID, 30)   // Developer
	auth.SetGroup(domain.GitLabGroup{ID: groupID, Name: "Backend", FullPath: "company/backend"})
}

func TestTeamService_Create_Maintainer(t *testing.T) {
	svc, _, auth := setupTeamSvc()
	seedAuth(auth)
	...
	if team.GitLabGroupPath != "company/backend" {
		t.Errorf("GitLabGroupPath = %q, want %q", team.GitLabGroupPath, "company/backend")
	}
}
...
func TestTeamService_Delete_Maintainer(t *testing.T) {
	...
	_, err := svc.Create(context.Background(), &domain.Team{
		Name: "To Delete", Slug: "to-delete", GitLabGroupID: groupID,
	}, maintainerToken)
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
...
func TestTeamService_Delete_DeveloperForbidden(t *testing.T) {
	...
	_, _ = svc.Create(context.Background(), &domain.Team{
		Name: "Protected", Slug: "protected", GitLabGroupID: groupID,
	}, maintainerToken)
