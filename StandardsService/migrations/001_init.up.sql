-- =============================================================================
-- 001_init.up.sql
-- Начальная схема БД для standards-service и cr-assistant
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS teams (
    ...
    gitlab_group_id    INTEGER     NOT NULL,
...
CREATE TABLE IF NOT EXISTS repositories (
    ...
    excludes    TEXT        NOT NULL DEFAULT '',  -- glob-паттерны через запятую
...
-- -----------------------------------------------------------------------------
-- code_standards
-- Один стандарт на команду. active_version_id — FK на standard_versions,
-- добавляется после создания таблицы versions (отложенный FK).
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS code_standards (
    id                UUID PRIMARY KEY,
    team_id           UUID        NOT NULL UNIQUE REFERENCES teams(id) ON DELETE CASCADE,
    active_version_id UUID,       -- FK добавляется ниже
    created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at        TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
